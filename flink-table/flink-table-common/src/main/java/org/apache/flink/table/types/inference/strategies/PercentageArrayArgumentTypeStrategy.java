...
public final class PercentageArrayArgumentTypeStrategy implements ArgumentTypeStrategy {
    ...
    public PercentageArrayArgumentTypeStrategy(boolean isNullable) {
        this.expectedNullability = isNullable;
    }
    ...
    public Optional<DataType> inferArgumentType(
        ...
        if (callContext.isArgumentLiteral(argumentPos)) {
            ...
            if (!literalVal.isPresent()
                    || Arrays.stream(literalVal.get())
                            .anyMatch(v -> v.doubleValue() < 0.0 || v.doubleValue() > 1.0)) {
                return callContext.fail(
                        throwOnFailure,
